...
You will need an existing disk0 image to boot from, with the RSHARD drivers on it (see the RSHARD documentation). These can be found Tim Mann’s site. It's worth a good look at this site, the drivers are in the download section.

You will also need to have a HDV hard disk image. 
 and a hard disk image. The easy way to
create the hard disk image is to use an emulator (trs80gp for example). You can also use the pre-setup files in the ldosboothd.zip, unzip them to the SD-Card and edit the boot.cfg file, it should just contain the name of the ini file to boot, in this case ldosboothd.ini Boot the model 1 in the normal way, turn the floppy-80 M1 on first with the SD-Card in it. Then turn on the machine, it should boot LSDOS.

To continue setting up manually:

There aren't many ways to create a hard disk hdd image. In fact the only real way at present is too use the trs80gp emulator. The only other application I have seen was a utility called mkdisk from strs, but this seems to fail with buffer overflow errors. So to create the disk do the following:
...
To boot on the floppy-80 M1 use the following ini file:
ldosboothd.ini 

Drive0=ldosboot2s.dmk

HD0=ldosboothd.hdv

And make sure HDV is enabled in system.cfg and boot.cfg should be set to use the correct ini file.


If you don't want to create a setup manually as above. You can
